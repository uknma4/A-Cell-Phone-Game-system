...
void EquipBag::OnEquipSmelt(int type, std::vector<int64_t> &itemIdList, Actor*actor)
{
	bool bSmelt = false;
	int goldNum = 0;
	int yuanbaoNum = 0;
	...
	for (int i = 0; i < count; ++i)
	{
		...
		if (smeltConfig)
		{
			...
			//获得装备就不能获得金币和强化石奖励和元宝奖励
			if (qualityAdd) continue;
			
			//给金币和强化石奖励
			goldNum = goldNum + smeltConfig->goldNum;// +floor(smeltConfig->goldNum * precent / 100);
	 
			int zhuanshenglevel = actor->GetBasicData()->zhuansheng_lv; //add by eddie 增加元宝奖励
			if (quality <= Item::QualityType_Blue) {
			 
			

				if (zhuanshenglevel < 1) {
					yuanbaoNum = yuanbaoNum + (wrand(3));
				}
				else if (zhuanshenglevel >= 1 && zhuanshenglevel < 5) {
					yuanbaoNum = yuanbaoNum + (wrand(4));
				}
				else if (zhuanshenglevel >= 5 && zhuanshenglevel < 9) {
					yuanbaoNum = yuanbaoNum + (wrand(6));
				}
				else if (zhuanshenglevel >= 9) {
					yuanbaoNum = yuanbaoNum + (wrand(8));
				}

			}
			else if (quality >= Item::QualityType_Violet)
			{
				if (zhuanshenglevel < 1) {
					yuanbaoNum = yuanbaoNum + (rand() % 3);
				}
				else if (zhuanshenglevel >= 1 && zhuanshenglevel < 5) {
					yuanbaoNum = yuanbaoNum + (rand() % 4);
				}
				else if (zhuanshenglevel >= 5 && zhuanshenglevel < 9) {
					yuanbaoNum = yuanbaoNum + (rand() % 6);
				}
				else if (zhuanshenglevel >= 9) {
					yuanbaoNum = yuanbaoNum + (rand() % 8);
				}
	...
	if (yuanbaoNum) {
		actor_->ChangeYuanBao(yuanbaoNum, "equip smelt");
	}
