...
local function giveRewards(actor, id, record, config)
	if not record.recidx or not record.recidx[1] then 
...
--[[	print("eddie conf.serNum")
	table.print(conf.serNum)
	print("eddie cfg")
	table.print(cfg)
	 print("eddie gdata.useIdx"..gdata.useIdx)   --]]  
	if not cfg then
...
local function GetPerRandIndex(actor, id, conf, record)
	...
	--[[ print("eddie record.useIdx"..record.useIdx)--]]  
	if not record.useIdx then return end
	local cfg = conf.perNum[record.useIdx]
	--[[	print("eddie conf.perNum")
	table.print(conf.perNum)
	print("eddie cfg")
	table.print(cfg)--]]
...
-- 计算累积权重
function mulYBturntableCount ()
local cumulativeWeights = {}
local totalWeight = 0
  for _, entry in ipairs(ActivityBtnConfig[1]) do
    totalWeight = totalWeight + entry.Weights
    cumulativeWeights[#cumulativeWeights + 1] = (cumulativeWeights[#cumulativeWeights] or 0) + entry.Weights
  end

-- 生成随机数
local randomIndex = math.random() * totalWeight
  for i, weight in ipairs(cumulativeWeights) do
    if randomIndex <= weight then
       -- print("随机选取的MulYBRand值:", ActivityBtnConfig[1][i].MulYBRand)
		return  ActivityBtnConfig[1][i].MulYBRand
...
local function hut(id, config, actor, record, packet, conf)
	...
	if idx then
		...
		if gidx then
			...
			local sumRate = 0
			MulIndex = math.random(1, #(ActivityBtnConfig[9]))
			local MulYBRand=ActivityBtnConfig[9][MulIndex].MulYBRand
			for idx, rate in ipairs(AllItem) do
				...
				if rand < sumRate then
					...
					if rand<MulYBRand then
					 --print("eddie rand mulybrand "..rand.."MulIndex "..MulIndex.."MulYBRand "..MulYBRand)
					 isOpenMulYB=1
					end 
					break
				end
	...
	--print("eddie rand isOpenMulYB "..isOpenMulYB)
 	return isOpenMulYB
end
...
local rtFunc = {
	...
	[RType.once] = function(id, config, actor, record, packet, conf)
		...
		if count > 0 then --领奖
			...
			--抽奖
               ...
               activitysystem.GisOpenMltYB[LActor.getName(actor)]={isOpenMtlYB=hut(id, config, actor, record, packet, conf)}
			actorevent.onEvent(actor, aeJoinActivityId, id, 1)
			
		end
	 
		activitysystem.sendActivityData(actor, id,activitysystem.GisOpenMltYB[LActor.getName(actor)].isOpenMtlYB)
	end,
	[RType.tenc] = function(id, config, actor, record, packet, conf)
		...
		for i=1,counts do
			--抽奖
		 if 	hut(id, config, actor, record, packet, conf)==1 then
	     	isOpenMtlYB=1
		  else
		  end 
		...
		giveRewards(actor, id, record, config)
		--发消息
		activitysystem.sendActivityData(actor, id,isOpenMtlYB)
		record.recidx = nil
